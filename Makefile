...
TEXT1 		:= MegaETk
TEXT2 		:= MegaETk by ETk
TEXT3 		:= etk.scener.org
...
BUILD		:=	build
SOURCES	:=	gfx source data source/fat level_events
...
NAFILES		:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.na)))
GIFFILES	:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.gif)))
BMPFILES	:=	$(foreach dir,$(SOURCES),$(notdir $(wildcard $(dir)/*.bmp)))
 
export OFILES	:=	$(MAPFILES:.map=.o) $(NAFILES:.na=.o) $(RAWFILES:.raw=.o) $(PALFILES:.pal=.o) $(BINFILES:.bin=.o) $(PCXFILES:.pcx=.o) $(JPEGFILES:.jpg=.o) $(MODFILES:.mod=.o) $(GIFFILES:.gif=.o) $(BMPFILES:.bmp=.o)\
